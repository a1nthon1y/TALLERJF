...
- **Llegada al Taller** (`/chofer/reportar-llegada`): Ingresa kilometraje del tacómetro, ruta/origen y un campo libre de comentarios/incidencias. Tiene UNA sola sección opcional: **Trabajos en ruta** — el chofer marca qué partes se atendieron en campo (con costo estimado); el backend las registra como `REALIZADO` y resetea sus contadores ANTES de correr el motor predictivo, evitando alertas falsas. **NO hay un segundo formulario de solicitud al encargado** — fue eliminado porque duplicaba "Solicitar Mantenimiento" y confundía al chofer. El campo `comentarios` + la página de solicitud son los canales para avisar al encargado. **Este es el único lugar donde se actualiza el odómetro de la unidad.**
- **Solicitar Mantenimiento** (`/chofer/solicitar-mantenimiento`): Formulario mínimo — solo muestra la unidad (auto-seleccionada) y UN campo de texto libre "¿Qué necesita atención?". No pide procedencia, ni lista de requerimientos, ni kilometraje. El encargado coordina los detalles en persona. El `observaciones` se guarda como texto libre directamente.
